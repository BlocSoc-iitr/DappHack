 ...
 */
function getEVMChains(env, chains = [], parent) {
    checkEnv(env);
    ...
    const testnet = getTestnetChains(selectedChains, parent);
 ...
 * @returns {Chain[]} - The chain objects.
 */
function getTestnetChains(chains = [], parent) {
    const _path = path.join(__dirname, '../../chain-config/testnet.json');
    const _path_fil = path.join(__dirname, '../../chain-config-parent/testnet.json');
    let testnet = [];
    if (!chains.includes(parent) && fs.existsSync(_path)) {
